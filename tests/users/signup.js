...
const {
  deleteData, createVarChars, returnRandomValue, createEmailVarChar,
} = Test;
...
describe('Test endpoint at "/api/v1/auth/signup" to create a User with POST', () => {
  ...
  it('Should NOT create a User at "/api/v1/auth/signup" if user password is less than 8 characters or more than 128 characters in length', async () => {
    const testData = {
      fullName: 'Frank',
      email: 'mama@mail.com',
      password: '1234AOdBcd!',
    ...
    testData.password = returnRandomValue('hhh', createVarChars(200));
    const response = await chai.request(app).post('/api/v1/auth/signup').send(testData);
    expect(response).to.have.status(400);
    expect(response.body).to.be.an('object');
    expect(response.body).to.have.property('status').to.be.a('number').to.equal(400);
